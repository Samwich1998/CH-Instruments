...
data_Directory = "../NASA Project Cortisol/Tryptophan/tryptophan NIP/" # The Path to the Folder with the CSV Files
...
for i,CV_CSV_Data in enumerate(sorted(CV_CSV_Data_List)):    
    ...
    plt.plot(potential, baseline, label="Baseline")  
    plt.plot(potential, baselineCurrent, label="New")
    ...
    potentialEnds = potential[0:stopInitial+1] + potential[stopFinal:-1]
    currentEnds = current[0:stopInitial+1] + current[stopFinal:-1]
    
    # Fit the Peak with a Cubic Spline
    cs = CubicSpline(potentialEnds, currentEnds)
    #cs = UnivariateSpline(potentialEnds, currentEnds)
    ...
    currentAxes.axvline(x=Vp, ymin=normalize(0, currentAxes.get_ylim()[0], currentAxes.get_ylim()[1]), ymax=normalize(float(max(baselineCurrent)), currentAxes.get_ylim()[0], currentAxes.get_ylim()[1]), linewidth=2, color='r', label="Peak Current: " + "%.4g"%Ip)
...
# Plot Curves
plt.title("Time Dependant DPV Peak Current: MIP")
